...
Texture::Texture(const std::string& file_path) : file_path_(file_path) {
  ...
  GL_CALL(glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, width_, height_, 0, GL_RGBA,
                       GL_UNSIGNED_BYTE, local_buffer_));
...
Texture::~Texture() { GL_CALL(glDeleteTextures(1, &renderer_id_)); }
